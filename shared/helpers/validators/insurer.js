...
/**
 * Checks whether an insurer ID is valid (exists in our database)
 *
 * @param {number} id - The ID of the insurer
 * @returns {boolean} - True if valid, false otherwise
 */
module.exports = async function(id){
	if(positive_integer.test(id)){
		let had_error = false;
		const sql = `SELECT COUNT(\`id\`) FROM \`#__insurers\` WHERE \`id\` = ${db.escape(parseInt(id, 10))};`;
