        ...
        for(const info of Object.values(locations)){
            ...
            for(const[activity_code, payroll]of Object.entries(info.activity_codes)){
                const ncci_data = ncci_codes.filter(function(code){
                    return code.activity_codes.includes(activity_code) && code.territory.toUpperCase() === territory.toUpperCase();
                });

                // If the insurer does support the activity code for that territory, write the row
                if(ncci_data.length){
