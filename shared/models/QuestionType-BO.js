...
const tableName = 'clw_talage_question_types'
const skipCheckRequired = false;
module.exports = class QuestionTypeBO{
        ...
        return new Promise(async (resolve, reject) => {
                ...
                if(result && result.length > 0 ){
                    for(let i=0; i < result.length; i++ ){
                        let questionTypeBO = new QuestionTypeBO();
                        await questionTypeBO.#dbTableORM.decryptFields(result[i]);
                        await questionTypeBO.#dbTableORM.convertJSONColumns(result[i]);
                        const resp = await questionTypeBO.loadORM(result[i], skipCheckRequired).catch(function(err){
                            log.error(`getList error loading object: ` + err + __location);
                        })
                        if(!resp){
                            log.debug("Bad BO load" + __location)
                        }
                        boList.push(questionTypeBO);
                    }
                    resolve(boList);
                }
                else {
    ...
    async getSelectionList(){
        ...
        const sql = `select id, name, logo  
            from clw_talage_question_types
            where state > 0
            order by name`
        const result = await db.query(sql).catch(function (error) {
            // Check if this was
