...
const tableName = 'clw_talage_industry_code_associations'
const skipCheckRequired = false;
module.exports = class IndustryCodeAssociationBO{
    ...
    getList(queryJSON) {
        return new Promise(async (resolve, reject) => {
                ...
                hasWhere = true;
                // Run the query
                log.debug("IndustryCodeAssociationBO getlist sql: " + sql);
                ...
                if(result && result.length > 0 ){
                    for(let i=0; i < result.length; i++ ){
                        let industryCodeAssociationBO = new IndustryCodeAssociationBO();
                        await industryCodeAssociationBO.#dbTableORM.decryptFields(result[i]);
                        await industryCodeAssociationBO.#dbTableORM.convertJSONColumns(result[i]);
                        const resp = await industryCodeAssociationBO.loadORM(result[i], skipCheckRequired).catch(function(err){
                            log.error(`getList error loading object: ` + err + __location);
                        })
                        if(!resp){
                            log.debug("Bad BO load" + __location)
                        }
                        boList.push(industryCodeAssociationBO);
                    }
                    resolve(boList);
                }
                else {
    ...
    delete(industryCodeId, activityCodeId) {
        return new Promise(async (resolve, reject) => {
            //validate
            if(industryCodeId > 0 && activityCodeId > 0){
                //Remove old records.
                // TODO: no state on this table?
                const sql =`DELETE FROM ${tableName} 
                        WHERE industryCodeId = ${db.escape(industryCodeId)}
                        AND activityCodeId = ${db.escape(activityCodeId)}
                `;
                let rejected = false;
                const result = await db.query(sql).catch(function (error) {
                    // Check if this was
