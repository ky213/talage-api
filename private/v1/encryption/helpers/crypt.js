...
exports.decrypt = function(val){
	...
	// Decrypt
	val = sodium.crypto_secretbox_open(val, nonce, Buffer.from(process.env.ENCRYPTION_KEY));
...
exports.encrypt = function(val){
	return new Promise(async function(fulfill, reject){
		...
		// Encrypt
		val = sodium.crypto_secretbox(Buffer.from(val), nonce, Buffer.from(process.env.ENCRYPTION_KEY));
