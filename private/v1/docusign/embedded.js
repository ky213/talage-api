...
async function PostEmbedded(req, res, next) {
	...
	const envelopeSummary = await envelopesApi.createEnvelope(accountId, {
		...
		error = ServerInternalError(errorMessage);
	});
	...
	if (!viewUrl) {
		...
		return next(ServerInternalError('We were unable to generate your document for signing at this time. Someone will contact you to complete these documents and open access to your account.'));
	}
...
exports.RegisterEndpoint = (basePath) => {
	ServerAddPost('Create Embedded DocuSign Document', basePath + '/embedded', PostEmbedded);
};