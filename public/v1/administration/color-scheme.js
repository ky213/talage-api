...
async function readColorSchemeList(req, res, next) {
	...
	try {
		...
		return server.sendError(res, next, 'Internal Error');
	}
	...
	// Send the response
	return server.send(colorSchemes, res, next);
}

exports.registerEndpoint = (server, basePath) => {
	// We require the 'administration.read' permission
	server.addGetAuth('Read color scheme list', `${basePath}/color-scheme`, readColorSchemeList, 'administration', 'all');
};
