...
async function GetLandingPage(req, res, next) {
	...
	if (!Object.prototype.hasOwnProperty.call(req.query, 'page') || !req.query.page) {
		...
		return next(new RestifyError.BadRequestError('You must specify a page'));
	}
	...
	if (landingPage.length !== 1) {
		...
		return next(new RestifyError.BadRequestError('Page not found'));
	}
...
exports.RegisterEndpoint = (basePath, server) => {
	server.get({
		...
		'path': basePath + '/landing-page'
	}, GetLandingPage);

	server.post({
		'name': 'Post Landing Page',
		'path': basePath + '/landing-page'
	}, PostLandingPage);
