...
async function getAgencyMetadata(req, res, next) {
    ...
    // get the landing page data so the header knows if there will be an about section
    const agencyLandingPageBO = new AgencyLandingPageBO();
    let landingPageJson = null;
    try{
        landingPageJson = await agencyLandingPageBO.getbySlug(agencyJson.id, pageSlug, !pageSlug);
    }
    catch (err) {
        log.warn(`Error retrieving agencyLandingPageBO in api agency ${agencySlug} (${pageSlug ? 'page ' + pageSlug : 'no page'}) url ${req.query.url}: ${err} ${__location}`);
    }
    ...
    res.send(200, {
        ...
        metaHasAbout: Boolean(landingPageJson) && Boolean(landingPageJson.about)
    });
    return next();
}
