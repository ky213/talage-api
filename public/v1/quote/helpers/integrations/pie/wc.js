    ...
    _insurer_quote() {
		...
		*/
        ...
        return new Promise(async(fulfill) => {
            ...
            const token_response = await this.send_request(host, '/connect/token', token_request_data, {'Content-Type': 'application/x-www-form-urlencoded'}).catch((err) => {
                log.error(`Pie WC: to get token error ${err}` + __location)
                had_error = true;
            ...
            await this.send_json_request(host, '/api/v1/Quotes', JSON.stringify(data), {Authorization: token}).
                then((res) => {
                    ...
                    try {
                        ...
                        log.error(`Pie WC: Error getting amount ${e} ` + __location)
                    ...
                    log.error(`Pie WC: Error  ${e} ` + __location)
