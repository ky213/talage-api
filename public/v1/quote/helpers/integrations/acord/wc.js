...
let email_body = 'Attached is the ACORD application from TEMPLATE_AGENCY_NAME for a Worker\'s Compensation policy';
...
module.exports = class ACORDWC extends Integration{
    ...
    async _insurer_quote(){
        ...
        // Generate acord
        let error = null;
        const generated_acord = await acordsvc.create(this.app.id, this.insurer.id, 'wc').catch(function(err){
            error = err;
        });
        ...
        if(acord_email === false){
            log.error(`Appid: ${this.app.id} Failed to retrieve Email for agency location ${this.app.agencyLocation.id}, insurer ${this.insurer.id} for WC` + __location);
            return this.return_result('error');
        }
    ...
    async getEmail(){
        ...
        try{
            ...
            log.error(`Appid: ${this.app.id} Missing acord email address WC agency location id ${this.app.agencyLocation.id} insurer: ${this.insurer.id}` + __location)
        }
        ...
        if(email_address && email_address.length > 0){
            ...
            log.error(`Appid: ${this.app.id} Email for agency location ${this.app.agencyLocation.id}, insurer ${this.insurer.id}, policy type WC, was not found.` + __location);
            return false;
        }
    }
}