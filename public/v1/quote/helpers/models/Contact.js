	...
	validate(){
		return new Promise((fulfill, reject) => {
			...
			if(email_result !== true){
				reject(new RestifyError.BadRequestError(email_result));
				return;
			}
			...
			if(this.first_name){
				if(!validator.isName(this.first_name)){
					reject(new RestifyError.BadRequestError('Invalid characters in first_name'));
					return;
				}

				if(this.first_name.length > 30){
					reject(new RestifyError.BadRequestError('First name exceeds maximum length of 30 characters'));
					return;
				}
			}else{
				reject(new RestifyError.BadRequestError('Missing required field in contact: first_name'));
				return;
			}
			...
			if(this.last_name){
				if(!validator.isName(this.last_name)){
					reject(new RestifyError.BadRequestError('Invalid characters in last_name'));
					return;
				}

				if(this.last_name.length > 30){
					reject(new RestifyError.BadRequestError('Last name exceeds maximum length of 30 characters'));
					return;
				}
			}else{
				reject(new RestifyError.BadRequestError('Missing required field in contact: last_name'));
				return;
			}
			...
			if(this.phone){
				...
				reject(new RestifyError.BadRequestError('Phone number is required'));
				return;
			}
