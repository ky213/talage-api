		...
		return new Promise(async(fulfill, reject) => {
			...
			if(this.address){
				...
				if(this.address.length > 100){
					reject(new RestifyError.BadRequestError('Address exceeds maximum of 100 characters'));
					return;
				}
			}else{
				reject(new RestifyError.BadRequestError('Missing required field: address'));
				return;
			}
			...
			if(this.address2){
				...
				if(this.address2.length > 20){
					reject(new RestifyError.BadRequestError('Address exceeds maximum of 20 characters'));
					return;
				}
			}
			...
			if(this.app.has_policy_type('WC')){
				if(this.activity_codes.length){
					...
					reject(new RestifyError.BadRequestError('At least 1 class code must be provided per location'));
					return;
				}
			}
			...
			if(this.identification_number){
				if(validator.ein(this.identification_number)){
					...
					reject(new RestifyError.BadRequestError('Invalid formatting for property: identification number.'));
					return;
				}
				...
				reject(new RestifyError.BadRequestError('Identification Number is required'));
				return;
			}
			...
			if(isNaN(this.full_time_employees) || this.full_time_employees < 0 || this.full_time_employees > 255){
				reject(new RestifyError.BadRequestError('full_time_employees must be an integer between 0 and 255 inclusive'));
				return;
			}
			...
			if(isNaN(this.part_time_employees) || this.part_time_employees < 0 || this.part_time_employees > 255){
				reject(new RestifyError.BadRequestError('part_time_employees must be an integer between 0 and 255 inclusive'));
				return;
			}
			...
			if(this.app.has_policy_type('BOP')){
				if(!validator.isSqFtg(this.square_footage) || this.square_footage < 100 || this.square_footage > 99999){
					return reject(new RestifyError.BadRequestError('square_footage must be an integer between 100 and 99,999 inclusive'));
				}
			}
			...
			if(this.app.has_policy_type('WC')){
				...
				if(unemployment_number_states.includes(this.territory)){
					if(this.unemployment_number === 0){
						reject(new RestifyError.BadRequestError(`Unemployment Number is required for all locations in ${unemployment_number_states.join(', ')}`));
						return;
					}
					if(!Number.isInteger(this.unemployment_number)){
						reject(new RestifyError.BadRequestError('Unemployment Number must be an integer'));
						return;
					}
				}else{
