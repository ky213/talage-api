...
module.exports = class Question{
	...
	set_answer(answer_id){
		return new Promise((fulfill, reject) => {
			...
			if(!this.id){
				...
				reject(new RestifyError.BadRequestError(`Invalid answer provided for Question ${this.id}. (${htmlentities.decode(this.text)})`));
				return;
			}
	...
	validate(){
		return new Promise((fulfill, reject) => {
			...
			if(!this.answer_id){
				reject(new RestifyError.BadRequestError(`Answer to question ${this.id} is invalid. (${type_help})`));
				return;
			}
