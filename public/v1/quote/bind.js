...
const QuoteBind = require('./helpers/models/QuoteBind.js');
...
async function PutBind(req, res, next){
    ...
    // Load
    await quoteBind.load(req.body.quote, req.body.payment_plan).catch(function(err){
        error = err;
    ...
    // Bind
    await quoteBind.bind().catch(function(err){
        error = err;
