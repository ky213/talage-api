...
async function getSettings(req, res, next) {
	...
	await crypt.batchProcessObject(settings, 'decrypt', ['ca_license_number', 'email', 'fname', 'lname', 'phone', 'website']);
	...
	if (locations.length) {
		...
		await crypt.batchProcessObjectArray(locations, 'decrypt', ['address', 'address2', 'email', 'fname', 'lname', 'phone']);
	...
	for (const locationID in locations) {
		if (Object.prototype.hasOwnProperty.call(locations, locationID)) {
			...
			if (Object.prototype.hasOwnProperty.call(location, 'close_time')) {
				if (![3, 4, 5, 6, 7, 8].includes(location.close_time)) {
			...
			if (Object.prototype.hasOwnProperty.call(location, 'open_time')) {
				if (![7, 8, 9, 10, 11].includes(location.open_time)) {
